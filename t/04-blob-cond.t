...
ok($dbh->do('create table q4m_t (id int not null,data mediumblob) engine=queue'));
my $data16k = '0123456789abcdef' x 1024;

ok($dbh->do('insert into q4m_t values (?,?)', {}, -1, $data16k));

for (my $i = 0; $i < 1000; $i++) {
    ok($dbh->do('insert into q4m_t values (?,?)', {}, $i, $data16k));
    is_deeply(
